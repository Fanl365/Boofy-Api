...
import { default as BoofyPriceMulticall } from '../abis/BoofyPriceMulticall.ts';
...
const fetchChainPools = withTracing(
  async (chain: ChainId, pools: Pool[]): Promise<PoolData[]> => {
    ...
    const multicallContract = fetchContract(multicallAddress, BoofyPriceMulticall, chain);
