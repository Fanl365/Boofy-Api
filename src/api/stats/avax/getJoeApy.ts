...
const getJoeApy = async () => {
  ...
  const boofyPerformanceFee = getTotalPerformanceFeeForVault(pool.name);
  const shareAfterBoofyPerformanceFee = 1 - boofyPerformanceFee;

  const simpleApr = yearlyRemittedUsd.dividedBy(totalStakedInUsd);
  const vaultApr = simpleApr.times(shareAfterBoofyPerformanceFee);
  const vaultApy = compound(simpleApr.toNumber(), DAILY_HPY, 1, shareAfterBoofyPerformanceFee);
  ...
  const apyBreakdowns = {
    [pool.name]: {
      ...
      boofyPerformanceFee: boofyPerformanceFee,
      vaultApy: vaultApy,
