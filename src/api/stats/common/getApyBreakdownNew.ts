...
export type ApyBreakdownRequest = BreakdownRequestComponents & {
  ...
  boofyFee?: BigNumberish | null | undefined;
...
export function getApyBreakdownOnly(request: ApyBreakdownRequest): ApyBreakdown {
  ...
  const boofyPerformanceFee = toNumber(request.boofyFee, getTotalPerformanceFeeForVault(request.vaultId));
  const shareAfterBoofyPerformanceFee = 1 - boofyPerformanceFee;
  ...
  for (const component of compoundableComponents) {
    ...
    if (apr !== undefined) {
      const aprAfterFee = apr * shareAfterBoofyPerformanceFee;
  ...
  // @dev shareAfterBoofyPerformanceFee = 1 as fee is already removed from all components
  breakdown.totalApy =
