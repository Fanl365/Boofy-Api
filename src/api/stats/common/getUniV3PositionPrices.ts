...
import BoofyUniswapPositionHelperAbi from '../../../abis/BoofyUniswapPositionHelper.ts';
...
const getPoolData = async (params: UniV3PositionPricesParams): Promise<PositionTokens[]> => {
  const boofyHelperContract = fetchContract(params.boofyHelper, BoofyUniswapPositionHelperAbi, params.chainId) as {
    read: {
      getPositionTokens: (args: [bigint, Address, Address]) => Promise<PositionTokens>;
    };
  };
  const calls = params.pools.map(pool =>
    boofyHelperContract.read.getPositionTokens([
      BigInt(pool.nftId),
