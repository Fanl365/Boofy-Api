...
import { getBoofyRewardPoolV2Apr } from './getBoofyRewardPoolV2Apr.ts';
...
const getCowRewardPoolApr = async (
  ...
  try {
    const rewardPoolData = await getBoofyRewardPoolV2Apr(chainId, {
    ...
    if (!rewardPoolData) {
      logger.warn({ chain: chainId, vault: clm.rewardPool.oracleId }, 'getBoofyRewardPoolV2Apr returned undefined');
      return result;
    }

    const { totalApr } = rewardPoolData;
    if (totalApr === undefined) {
      logger.warn({ chain: chainId, vault: clm.rewardPool.oracleId }, 'getBoofyRewardPoolV2Apr returned no total apr');
      return result;
    }
