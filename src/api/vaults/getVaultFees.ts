...
const updateFeeBatch = async () => {
  ...
  const { boofyFeeRecipient: feeBatchAddress } = addressBookByChainId[chainId].platforms.boofyfinance;
...
const callHandlers = {
  ...
  boofy: makeBigIntToNumberHandler('boofyFee'),
  ...
  breakdown: async (contract: FeeAbiContract) => {
    ...
    return {
      ...
      boofy: Number(value.boofy / BigInt(1e9)) / 1e9,
  ...
  allFees: async (contract: FeeAbiContract) => {
    ...
    return {
      performance: {
        total: Number(value.boofy.total / BigInt(1e9)) / 1e9,
        boofy: Number(value.boofy.boofy / BigInt(1e9)) / 1e9,
        call: Number(value.boofy.call / BigInt(1e9)) / 1e9,
        strategist: Number(value.boofy.strategist / BigInt(1e9)) / 1e9,
      },
...
const legacyFeeMappings = (
  ...
  const boofyFee = orNaN(contractCalls.boofy);
  ...
  if (callFee + strategistFee + boofyFee === maxFee) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
      strategist: (total * strategistFee) / maxFee,
      treasury: (total * feeBatch.treasurySplit * boofyFee) / maxFee,
      stakers: (total * feeBatch.stakerSplit * boofyFee) / maxFee,
    };
  ...
  } else if (!isNaN(strategistFee + callFee + boofyFee)) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
      strategist: (total * strategistFee) / maxFee,
  ...
  } else if (callFee + boofyFee === maxFee) {
...
const performanceFromGetFees = (fees: PerformanceFeeCallResponse, feeBatch: FeeBatchDetail): PerformanceFee => {
  ...
  let boofy = fees.boofy;
  let call = fees.call;
  let strategist = fees.strategist;

  const feeSum = boofy + call + strategist;
  const boofySplit = (total * boofy) / feeSum;

  return {
    total: total,
    call: (total * call) / feeSum,
    strategist: (total * strategist) / feeSum,
    treasury: feeBatch.treasurySplit * boofySplit,
    stakers: feeBatch.stakerSplit * boofySplit,
  };
};
