...
import type { BoofyFinance } from '@beefyfinance/blockchain-addressbook/types/beefyfinance';
...
const configsByChain: Record<string, Partial<BoofyFinance>> = {};

export const initConfigService = () => {
  typedKeys(addressBook).forEach(chain => {
    const config = addressBook[chain].platforms.boofyfinance;
    // Prune ab fields
