      ...
      await Promise.all(
        rewardersForVaults.map(async rewarder => {
          ...
          return rewardInfos.map(reward => {
            ...
            if (rewardActive && (!tokenApr || !isFiniteNumber(tokenApr.boofyApr))) {
              logger.warn({ token: tokenAddress, pool: rewarder.poolAddress }, 'missing boofyApr for active reward');
            }
            const apr = tokenApr && rewardActive ? toNumber(tokenApr.boofyApr, 0) / 100 : 0;
