...
export const getBoosts = async (chain: ApiChain): Promise<BoostEntity[]> => {
  const promosEndpoint = `https://raw.githubusercontent.com/boofyfinance/boofy-v2/prod/src/config/promos/chain/${toAppChain(
    chain
  )}.json`;
...
export const getBoostPeriodFinish = async (chain: ApiChain, boosts: BoostEntity[]): Promise<Boost[]> => {
  ...
  const periodFinishCalls = boosts.map(async (boost): Promise<number[]> => {
    if (boost.version >= 2) {
      const poolContract = fetchContract(boost.contractAddress, IBoofyRewardPool, chainId);
    ...
    const boostContract = fetchContract(boost.contractAddress, BoofyBoostAbi, chainId);
