...
async function main() {
  ...
  // FIXME(unsafe-cast): unchecked response shape
  const tvlByChain = (await fetch('https://api.boofy.finance/tvl').then(r => r.json())) as TvlByChainApiResponse;
  const tvl = Object.keys(tvlByChain)
    .map(k => tvlByChain[k])
    .reduce<Record<string, number>>((p, c) => ({ ...p, ...c }), {});
  const holderCounts = await fetch('https://balance-api.boofy.finance/api/v1/holders/counts/all').then(r => r.json());
