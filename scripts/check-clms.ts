...
/**
 * This script checks the boofyCowVaults.json configs against the on-chain contracts.
...
async function start() {
  ...
  const clmFiles = (await fg(['./src/data/**/boofyCowVaults.json'])).filter(path =>
...
function extractChainIdFromPath(path: string): ApiChain {
  const matches = path.match(/\/([^\/]+)\/boofyCowVaults\.json$/);
