...
function addChain() {
  ...
  const boofyCowVaultsFile = path.join(chainPath, 'boofyCowVaults.json');
  fs.writeFileSync(boofyCowVaultsFile, '[]');
  ...
  fs.writeFileSync(
    chainIndexFile,
    `
        import { getBoofyCow${chainNameCapitalized}Apys } from './getBoofyCow${chainNameCapitalized}Apys.ts';

        const getApys = [getBoofyCow${chainNameCapitalized}Apys];
  ...
  const vaultsEndpointStatement = `const ${chainName.toUpperCase()}_VAULTS_ENDPOINT =
   'https://raw.githubusercontent.com/boofyfinance/boofy-v2/prod/src/config/vault/${chainName}.json';`;

  // Find the Vaults Endpoints section
  const vaultsEndpointsRegex = /\/\/ Boofy Vaults Endpoints[\s\S]*?(?=\n\n)/;
...
`;
  ...
  const multicallsWeb3Regex =
    /const MULTICALLS: Record<ChainId, Pick<BoofyFinance, 'multicall'>\['multicall'\]> = {[\s\S]*?};/;
  web3HelpersContent = web3HelpersContent.replace(multicallsWeb3Regex, match => {
    return (
      match.slice(0, -2)
      + `  [ChainId.${chainName}]: addressBookByChainId[ChainId.${chainName}].platforms.boofyfinance.multicall,\n};`
    );
  });
